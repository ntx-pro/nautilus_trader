...
/// encoding parameter (typically -8), not the instrument's trading precision. The actual
/// precision is determined by the mantissa's trailing zeros.
///
/// # Examples
///
...
mod tests {
    ...
    mod sbe_precision_tests {
        ...
        #[rstest]
        #[case::ethusdc(1_000_000, -8, 2)]
        #[case::btcusdt(1_000_000, -8, 2)]
        #[case::dotusdt(100_000, -8, 3)]
        #[case::adausdt(10_000, -8, 4)]
        #[case::dogeusdt(1_000, -8, 5)]
        #[case::ltcbtc(100, -8, 6)]
        #[case::gasbtc(10, -8, 7)]
        #[case::shibusdt(1, -8, 8)]
        #[case::btctry(100_000_000, -8, 0)]
        #[case::bchusdt(10_000_000, -8, 1)]
        fn test_sbe_tick_precision(
            #[case] mantissa: i64,
            #[case] exponent: i8,
            #[case] expected: u8,
        ) {
            ...
            assert_eq!(
                result, expected,
                "mantissa={mantissa}, exponent={exponent}: expected precision={expected}, got={result}"
            );
        }

        #[rstest]
        fn test_sbe_tick_precision_zero_mantissa() {
            assert_eq!(sbe_mantissa_precision(0, -8), 0);
        }

        #[rstest]
        fn test_sbe_tick_precision_positive_exponent() {
            // Exponent >= 0 means integer values, precision = 0
        ...
        fn test_parse_sbe_price_filter_ethusdc() {
            let filter = BinancePriceFilterSbe {
                price_exponent: -8,
                min_price: 1_000_000,    // 0.01
                max_price: 100_000_000_000_000, // 1_000_000
                tick_size: 1_000_000,    // 0.01
            };
            ...
            assert_eq!(max_price.unwrap().precision, 2, "max_price precision");
            assert_eq!(min_price.unwrap().precision, 2, "min_price precision");
        }
        ...
        fn test_parse_sbe_price_filter_shibusdt() {
            let filter = BinancePriceFilterSbe {
                price_exponent: -8,
                min_price: 1,            // 0.00000001
                max_price: 100_000_000,  // 1.0
                tick_size: 1,            // 0.00000001
            };

            let (tick_size, _, _) = parse_sbe_price_filter(&filter).unwrap();

            assert_eq!(tick_size.precision, 8, "SHIBUSDT should have precision=8");
        ...
        fn test_parse_sbe_lot_size_filter_ethusdc() {
            let filter = BinanceLotSizeFilterSbe {
                qty_exponent: -8,
                min_qty: 10_000,         // 0.0001
                max_qty: 900_000_000_000, // 9000.0
                step_size: 10_000,       // 0.0001
            };

            let (step_size, max_qty, min_qty) = parse_sbe_lot_size_filter(&filter).unwrap();

            assert_eq!(step_size.precision, 4, "step_size precision");
            assert_eq!(min_qty.unwrap().precision, 4, "min_qty precision");
            assert_eq!(max_qty.unwrap().precision, 4, "max_qty precision");
        }
    }
}
